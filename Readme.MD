...
| S7Server PLC 模拟器 | ⚠️ 部分实现 | 握手已通，DB内存存储待完善 |
| 类型化读写 API（Read〃T〃/Write） | ⬜ 待开发 | 自动字节序转换 |
| 批量多变量读写（ReadMultipleVars） | ⬜ 待开发 | 单次 PDU 打包多个变量 |
| SZL 诊断读取（CPU 状态、模块信息） | ⬜ 待开发 | 设备健康状态查询 |
| PLC 时钟读写 | ⬜ 待开发 | 同步 NTP 时间 |
